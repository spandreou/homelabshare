...
export default function Lightfall({
  ...
  const colorsKey = colors.slice(0, MAX_COLORS).join("|");
  ...
  useEffect(() => {
    ...
    const { arr, count, avg } = prepColors(colorsKey.split("|").filter(Boolean));
    const uniforms = {
    ...
    } satisfies Record<string, Uniform<number | number[] | [number, number, number]>>;
